<script lang="ts">
	...
	function performSearch() {
		...
		goto(`${BASE_PATH}?${params.toString()}`, { keepFocus: true });
	}
...
<svelte:head>
	<title>Agent Prompt Library | Bearprompt</title>
	<meta name="description" content="Browse and discover AI agent prompts for Notion AI, Microsoft Copilot, Cursor, Claude Code, and more. Add agent prompts to your local library." />
	<link rel="canonical" href="https://bearprompt.com/agents" />
	
	<!-- Open Graph -->
	<meta property="og:title" content="Agent Prompt Library | Bearprompt" />
	<meta property="og:description" content="Browse and discover AI agent prompts for Notion AI, Microsoft Copilot, Cursor, Claude Code, and more. Add agent prompts to your local library." />
	<meta property="og:url" content="https://bearprompt.com/agents" />
	<meta property="og:type" content="website" />
	<meta property="og:image" content="https://bearprompt.com/og-image.png" />
	
	<!-- Twitter -->
	<meta name="twitter:card" content="summary_large_image" />
	<meta name="twitter:title" content="Agent Prompt Library | Bearprompt" />
	<meta name="twitter:description" content="Browse and discover AI agent prompts for Notion AI, Microsoft Copilot, Cursor, Claude Code, and more. Add agent prompts to your local library." />
	...
	<div class="mb-8">
		<div class="flex gap-2">
			<div class="relative flex-1">
				...
				<input
					type="text"
					placeholder="Search agent prompts..."
					value={searchInput}
	...
	{#if data.mode === 'search'}
		...
		{:then searchResults}
			...
			{:else}
				<div class="mb-4">
					<p class="text-sm" style="color: var(--color-text-secondary);">
						Found {searchResults.totalResults} agent{searchResults.totalResults === 1
							? ''
							: 's'}
				...
				<div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
					{#each searchResults.prompts as prompt}
						<PublicPromptCard {prompt} showAuthor={true} basePath={BASE_PATH} onAddToLibrary={handleAddToLibrary} />
					{/each}
				</div>
			...
			{:else}
				...
				{#if libraryData.highlightedAuthors.length === 0 && libraryData.authors.length === 0}
					<div class="py-12 text-center">
						<p style="color: var(--color-text-muted);">No agent prompts available yet.</p>
					</div>
				{/if}
