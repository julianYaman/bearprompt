<script lang="ts">
	...
	const metaDescription = $derived(
		data.author.public_description ||
			`Browse ${data.totalPrompts} AI agent prompt${data.totalPrompts === 1 ? '' : 's'} by ${data.author.name} on Bearprompt.`
	);

	// SEO: Generate page title
	const pageTitle = $derived(`${data.author.name} - Agent Prompts | Bearprompt`);
	...
	const jsonLd = $derived(
		JSON.stringify({
			...
			'@type': 'ProfilePage',
			name: `${data.author.name}'s Agent Prompts`,
...
<svelte:head>
	...
	<!-- Open Graph -->
	<meta property="og:title" content="{data.author.name} - Agent Prompts | Bearprompt" />
	...
	<header class="mb-8">
		<div class="flex items-start gap-4">
			...
			<div class="flex-1 min-w-0">
				...
				>
					{data.totalPrompts} agent prompt{data.totalPrompts === 1 ? '' : 's'}
				</p>
			</div>
		</div>
	</header>
	...
	{#if data.prompts.length > 0}
		<div class="grid grid-cols-1 gap-4 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
			{#each data.prompts as prompt}
				<PublicPromptCard {prompt} author={data.author} basePath={BASE_PATH} onAddToLibrary={handleAddToLibrary} />
			{/each}
		</div>
	{:else}
		<div class="py-16 text-center">
			<p style="color: var(--color-text-secondary);">
				This author hasn't published any agent prompts yet.
			</p>
		</div>
	{/if}
